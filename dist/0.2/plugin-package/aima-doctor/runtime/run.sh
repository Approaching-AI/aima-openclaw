#!/usr/bin/env bash
# AIMA Doctor runtime helper for packaged OpenClaw skill installs
#
# Run (TTY): ~/.openclaw/tools/aima-doctor/run.sh --run --terminal [--symptom "..."]
set -euo pipefail

DOCTOR_DIR="${HOME}/.openclaw/tools/aima-doctor"
API_BASE_URL=https://aimaservice.ai/platform/api/v1
...
RUN_MODE="run"        # packaged runtime executes immediately
...
if [ -f "${DOCTOR_DIR}/config.json" ]; then
